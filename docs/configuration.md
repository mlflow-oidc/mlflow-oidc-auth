...
Group patterns are evaluated locally against the groups already returned by the OIDC provider, so they do not add a provider API call to login. After a successful login, all groups from the claim are synchronized to the authorization database, including groups that did not participate in the access match. Prefer a scoped pattern such as `mlflow-*`; using `*` allows every identity with at least one group in the claim to log in.
