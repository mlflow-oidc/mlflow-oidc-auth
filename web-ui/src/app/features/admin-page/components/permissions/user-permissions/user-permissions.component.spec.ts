...
describe("UserPermissionsComponent", () => {
  ...
  beforeEach(async () => {
    await TestBed.configureTestingModule({
      ...
      providers: [
        { provide: ActivatedRoute, useValue: { params: of({}), snapshot: { paramMap: { get: () => null }, routeConfig: { path: "users" } } } },
        ...
        { provide: CreateServiceAccountService, useValue: {
  ...
  it("should handle item action with EDIT", () => {
    ...
    component.handleItemAction({ action: getAction(TableActionEnum.EDIT), item: getUser("u") });
    expect(component.handleUserEdit).toHaveBeenCalledWith({ username: "u" });
  });

  it("should not handle item action with unsupported action", () => {
    component.handleUserEdit = jest.fn();
    const unsupportedAction = { action: TableActionEnum.DELETE, icon: "icon", name: "name" };
    component.handleItemAction({ action: unsupportedAction, item: getUser("u") });
    expect(component.handleUserEdit).not.toHaveBeenCalled();
  });

  it("should handle service account action EDIT", () => {
    component.handleUserEdit = jest.fn();
    component.handleServiceAccountAction({ action: getAction(TableActionEnum.EDIT), item: getUser("svc") });
  ...
  it("should handle service account action DELETE", () => {
    ...
    component.handleServiceAccountAction({ action: getAction(TableActionEnum.DELETE), item: getUser("svc") });
    expect(component.handleServiceAccountDelete).toHaveBeenCalledWith({ username: "svc" });
  });

  it("should handle service account action GET_ACCESS_KEY", () => {
    component.handleAccessKey = jest.fn();
    component.handleServiceAccountAction({ action: getAction(TableActionEnum.GET_ACCESS_KEY), item: getUser("svc") });
  ...
  it("should not handle service account action with unsupported action", () => {
    component.handleUserEdit = jest.fn();
    const unsupportedAction = { action: TableActionEnum.ADD, icon: "icon", name: "name" };
    component.handleServiceAccountAction({ action: unsupportedAction, item: getUser("svc") });
    expect(component.handleUserEdit).not.toHaveBeenCalled();
  });
  ...
  it("should delete service account and reload data", () => {
    ...
    expect(userDataService.deleteUser).toHaveBeenCalledWith({ username: "svc" });
  ...
  it("should create service account and reload data", () => {
    const modalResult = { username: "svc3", display_name: "svc3", is_admin: false, is_service_account: false };
  ...
  it("should open access key dialog with correct data", () => {
    ...
    expect(dialog.open).toHaveBeenCalledWith(AccessKeyModalComponent, { data: { token: "tok123" } });
  });
});
