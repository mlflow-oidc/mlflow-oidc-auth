...
import {
  ...
  ModelUserListModel,
} from "src/app/shared/interfaces/models-data.interface";
import {
  PermissionEnum,
  PermissionTypeEnum,
...
describe("ModelsDataService", () => {
  ...
  it("getModelsForUser should perform GET to correct URL and map response", () => {
    const userName = "john";
    const expectedUrl = API_URL.MODELS_FOR_USER.replace(
      "${userName}",
      userName,
    );
  ...
  it("getUsersForModel should perform GET to correct URL", () => {
    ...
    const expectedUrl = API_URL.USERS_FOR_MODEL.replace(
      "${modelName}",
      modelName,
    );
    const mockUsers: ModelUserListModel[] = [
      { permission: PermissionEnum.EDIT, username: "alex" },
    ];
