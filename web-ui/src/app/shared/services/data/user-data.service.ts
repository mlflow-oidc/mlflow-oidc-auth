...
import { AllUsersListModel, CurrentUserModel, TokenModel, RegisteredModelPermission, ExperimentPermission, PromptPermission } from '../../interfaces/user-data.interface';
    ...
    return this.http.get<CurrentUserModel>(API_URL.GET_CURRENT_USER).pipe(
      switchMap(user => {
        ...
        return forkJoin({
          ...
          prompts: this.http.get<{ prompts: PromptPermission[] }>(API_URL.PROMPTS_FOR_USER.replace('${userName}', userName))
        }).pipe(
          map(response => {
            user.models = response.models.models;
            user.experiments = response.experiments.experiments;
            user.prompts = response.prompts.prompts;
            return { user, models: response.models.models, experiments: response.experiments.experiments, prompts: response.prompts.prompts };
          })
        );
      })
    );
