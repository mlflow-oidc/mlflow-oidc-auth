...
describe("RegexPermissionsView", () => {
    ...
    beforeEach(() => {
        ...
        vi.spyOn(useGroupExpHooks, "useGroupExperimentPatternPermissions").mockReturnValue(defaultHookResult as any);
        vi.spyOn(useGroupModelHooks, "useGroupModelPatternPermissions").mockReturnValue(defaultHookResult as any);
        vi.spyOn(useGroupPromptHooks, "useGroupPromptPatternPermissions").mockReturnValue(defaultHookResult as any);
    });
    ...
    it("renders loading and error", () => {
        vi.spyOn(useExpHooks, "useUserExperimentPatternPermissions").mockReturnValue({ ...defaultHookResult, isLoading: true } as any);
        const { rerender } = render(<RegexPermissionsView type="experiments" entityKind="user" entityName="user1" />);
        expect(screen.getByText(/Loading/i)).toBeDefined();

        vi.spyOn(useExpHooks, "useUserExperimentPatternPermissions").mockReturnValue({ ...defaultHookResult, error: "Fail" } as any);
        rerender(<RegexPermissionsView type="experiments" entityKind="user" entityName="user1" />);
