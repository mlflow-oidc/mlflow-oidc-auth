...
describe("API Endpoints", () => {
  ...
  describe("Dynamic Endpoints", () => {
    ...
    it("returns correct user experiment permissions URL", () => {
      expect(DYNAMIC_API_ENDPOINTS.USER_EXPERIMENT_PERMISSIONS("testuser")).toBe(
        "/api/2.0/mlflow/permissions/users/testuser/experiments",
      );
    });

    it("returns correct user experiment permission URL", () => {
      expect(DYNAMIC_API_ENDPOINTS.USER_EXPERIMENT_PERMISSION("testuser", "exp1")).toBe(
        "/api/2.0/mlflow/permissions/users/testuser/experiments/exp1",
      );
    });
    ...
    it("returns correct user model permission URL", () => {
      expect(DYNAMIC_API_ENDPOINTS.USER_MODEL_PERMISSION("testuser", "model1")).toBe(
    ...
    it("returns correct user prompt permission URL", () => {
      expect(DYNAMIC_API_ENDPOINTS.USER_PROMPT_PERMISSION("testuser", "prompt1")).toBe(
        "/api/2.0/mlflow/permissions/users/testuser/prompts/prompt1",
      );
    });

    it("returns correct pattern permissions URLs", () => {
      expect(DYNAMIC_API_ENDPOINTS.USER_EXPERIMENT_PATTERN_PERMISSIONS("u1")).toContain(
        "experiment-patterns",
      );
      expect(DYNAMIC_API_ENDPOINTS.USER_MODEL_PATTERN_PERMISSIONS("u1")).toContain(
        "registered-models-patterns",
      );
      expect(DYNAMIC_API_ENDPOINTS.USER_PROMPT_PATTERN_PERMISSIONS("u1")).toContain(
        "prompts-patterns",
      );
    });

    it("returns correct resource user permissions URLs", () => {
      expect(DYNAMIC_API_ENDPOINTS.EXPERIMENT_USER_PERMISSIONS("exp1")).toContain(
        "/experiments/exp1/users",
      );
    ...
    it("returns correct group permissions URLs", () => {
      expect(DYNAMIC_API_ENDPOINTS.GROUP_EXPERIMENT_PERMISSIONS("g1")).toContain(
        "/groups/g1/experiments",
      );
    ...
    it("returns correct group pattern permissions URLs", () => {
      expect(DYNAMIC_API_ENDPOINTS.GROUP_EXPERIMENT_PATTERN_PERMISSIONS("g1")).toContain(
        "experiment-patterns",
      );
      expect(DYNAMIC_API_ENDPOINTS.GROUP_MODEL_PATTERN_PERMISSION("g1", "pat1")).toContain(
        "registered-models-patterns/pat1",
      );
    });
    ...
    it("covers all remaining dynamic endpoints", () => {
      ...
      Object.values(DYNAMIC_API_ENDPOINTS).forEach((fn) => {
        if (typeof fn === "function") {
          ...
          const args: string[] = new Array(fn.length).fill("test") as string[];
          (fn as (...args: string[]) => string)(...args);
        }
      });
    });
  });
