...
describe("useUpdateWebhook", () => {
  ...
  it("calls updateWebhook and shows success toast on success", async () => {
    ...
    let success;
    await act(async () => {
      success = await result.current.update("123", { name: "New Name" }, {
    ...
    expect(webhookServiceModule.updateWebhook).toHaveBeenCalledWith("123", { name: "New Name" });
  ...
  it("calls updateWebhook and shows error toast on failure", async () => {
    vi.spyOn(webhookServiceModule, "updateWebhook").mockRejectedValue(new Error("API Error"));
