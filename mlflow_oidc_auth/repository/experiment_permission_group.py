...
class ExperimentPermissionGroupRepository(
    BaseGroupPermissionRepository[SqlExperimentGroupPermission, ExperimentPermission]
):
    model_class = SqlExperimentGroupPermission
    resource_id_attr = "experiment_id"

    # -- Backward-compatible alias for the private helper ---------------------
    _get_experiment_group_permission = (
        BaseGroupPermissionRepository._get_group_permission_or_none
    )

    # -- Alias matching original method name ----------------------------------

    def get_group_permission_for_user_experiment(
