...
def _search_logged_models_needs_no_filtering(resp: Response, username: str) -> bool:
    """Report whether the response body can be passed through untouched.
    ...
    workspaces: dict[str, str | None] | None = {} if config.MLFLOW_ENABLE_WORKSPACES else None
    for model in models:
        ...
        if workspaces is not None:
            if experiment_id not in workspaces:
                try:
                    workspaces[experiment_id] = _get_tracking_store().get_experiment(experiment_id).workspace
                except Exception:
                    workspaces[experiment_id] = None
            if not _can_access_workspace(username, workspaces[experiment_id]):
                return False
...
def _filter_search_logged_models(resp: Response) -> None:
    ...
    if _search_logged_models_needs_no_filtering(resp, username):
        return
