...
@catch_mlflow_exception
@check_admin_permission
def get_group_prompt_regex_permission(group_name):
    ep = store.list_group_prompt_regex_permissions(
        group_name=group_name,
    )
    return jsonify({"prompt_permission": [e.to_json() for e in ep]}), 200


@catch_mlflow_exception
@check_admin_permission
def update_group_prompt_regex_permission(group_name):
...
@catch_mlflow_exception
@check_admin_permission
def delete_group_prompt_regex_permission(group_name):
