...
def test_update(repo, session):
    ...
    with (
        patch("mlflow_oidc_auth.repository.experiment_permission_regex_group.get_group", return_value=group),
...
def test_list_permissions_for_user_groups(repo, session):
    ...
    with (
        patch("mlflow_oidc_auth.repository.experiment_permission_regex_group.get_user", return_value=user),
        patch("mlflow_oidc_auth.repository.experiment_permission_regex_group.list_user_groups", return_value=[group1, group2]),
    ):
...
def test_grant(repo, session):
    ...
    with (
        ...
        patch("mlflow_oidc_auth.repository.experiment_permission_regex_group._validate_permission"),
        patch("mlflow_oidc_auth.repository.experiment_permission_regex_group.validate_regex"),
        patch("mlflow_oidc_auth.repository.experiment_permission_regex_group.SqlExperimentGroupRegexPermission", return_value=perm),
    ):
