...
def test_delete_user_with_experiment_permissions_deletes_permissions_rows(
    ...
    store.create_experiment_permission(
        experiment_id="exp1", username=username, permission="READ"
    )
    ...
    with store.ManagedSessionMaker() as session:
        assert (
            session.query(SqlUser).filter(SqlUser.username == username).one_or_none()
            is None
        )
        assert (
            session.query(SqlExperimentPermission)
...
def test_delete_user_with_gateway_permissions_deletes_all_gateway_rows(
    ...
    store.create_gateway_endpoint_permission(
        gateway_name="ep1", username=username, permission="READ"
    )
    store.create_gateway_secret_permission(
        gateway_name="sec1", username=username, permission="READ"
    )
    store.create_gateway_model_definition_permission(
        gateway_name="md1", username=username, permission="READ"
    )
    ...
    with store.ManagedSessionMaker() as session:
        ...
        assert (
            ...
            .filter(SqlGatewayEndpointPermission.user_id == user_id)
            .count()
            == 1
        )
        assert (
            session.query(SqlGatewaySecretPermission)
            .filter(SqlGatewaySecretPermission.user_id == user_id)
            .count()
            == 1
        )
        assert (
            session.query(SqlGatewayModelDefinitionPermission)
            .filter(SqlGatewayModelDefinitionPermission.user_id == user_id)
            .count()
            == 1
        )
