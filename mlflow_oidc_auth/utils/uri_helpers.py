...
middleware configured, Flask's request object automatically contains the correct
...
Dependencies:
- Flask request context (requires active request)
...
from mlflow_oidc_auth.routes import CALLBACK
...
def _get_base_url_from_request() -> str:
    """
    Extract the base URL from the current Flask request context.
...
def _get_dynamic_redirect_uri(callback_path: str = CALLBACK) -> str:
    """
...
def get_configured_or_dynamic_redirect_uri(configured_uri: Optional[str], callback_path: str = CALLBACK) -> str:
    """
    ...
    return _get_dynamic_redirect_uri(callback_path)
